...
png(file.path(plots.folder, "sequences", "nofun", "seqdplot.png"), width=480, height=480)
...
png(file.path(plots.folder, "sequences", "nofun", "seqdplot2.png"), width=1200, height=480)
seqdplot(
...
################ stats sur les etats ################ 
sdb <- as.matrix(sd)						# on convertit sd en une matrice plus pratique ici
mandate.types <- sort(unique(c(sdb)))		# on récupère tous les types de mandats presents dans la matrice
if(any(mandate.types=="*"))
	mandate.types <- mandate.types[-which(mandate.types=="*")] 
# on init la table qui recevra les stats
stat.names <- c("Moyenne","Mediane")
state.stats <- matrix(NA, nrow=length(stat.names), ncol=length(mandate.types))
colnames(state.stats) <- mandate.types
rownames(state.stats) <- stat.names
# on traite chaque type de mandat successivement
for(mandate.type in mandate.types)
{	# on décompte ce type de mandat dans chaque ligne
	tmp <- apply(X=sdb, MARGIN=1, FUN=function(row) length(which(row==mandate.type)))
	# quelles sont les lignes contenant au moins une fois ce type de mandat ?
	idx <- which(tmp>0)
	# on calcule les stats uniquement pour ce type de mandat
	state.stats["Moyenne",mandate.type] <- mean(tmp[idx])
	state.stats["Mediane",mandate.type] <- median(tmp[idx])
	# tu peux meme avoir toute la distribution, c'est plus informatif
	png(file.path(plots.folder, "sequences", "nofun", paste0("etats_",mandate.type,"_distribution.png")))
		hist(tmp[idx], main=NA, xlab=paste0("Etats ",mandate.type), ylab="Frequence", col=STATE_COLORS[mandate.type])
	dev.off()
}
